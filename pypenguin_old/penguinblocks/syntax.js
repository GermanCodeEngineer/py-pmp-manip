...
function parseLines(code, languages) {
  ...
  return () => {
    ...
    // Debugging: log the line value before returning
    //console.log("Returned line value:", line);
...
function cleanScripts(scripts) {
  // Remove the language attribute, because the whole language data(huge) is included in EVERY SINGLE BLOCK
  scripts.forEach(script => {
    eachBlock(script, block => {
      const info = block.info;
      if (info && "language" in info) {
        delete block.info.language;
      }
    })
  })
}

...
def _26f9_8217(path: AbstractTreePath, block: SRBlock) -> None:
    ...
    if block.opcode == NEW_OPCODE_CB_DEF:
        if mutation.optype.is_reporter():
            raise GU_InvalidValueError(path, f"If mutation.optype of a {block.__class__.__name__} is any ...REPORTER optype, opcode should be {NEW_OPCODE_CB_DEF_REP!r}")
    elif block.opcode == NEW_OPCODE_CB_DEF_REP:
        if not mutation.optype.is_reporter():
            raise GU_InvalidValueError(path, f"If mutation.optype of a {block.__class__.__name__} is NOT any ...REPORTER optype, opcode should be {NEW_OPCODE_CB_DEF!r}")

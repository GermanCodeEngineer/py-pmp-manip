...
from pmp_manip.utility          import (
    grepr_dataclass, field, get_closest_matches, tuplify, listify, string_to_sha256, enforce_type,
...
class SRBlock(HasGreprValidate):
    ...
    def post_validate(self, 
        ...
        for new_input_id, input in self.inputs.items():
            ...
            enforce_type(input, input_cls, input_path, condition="For this opcode")
            input.validate(
                path          = input_path,
                info_api      = info_api,
                validation_if = validation_if,
...
class SRInputValue(ABC, HasGreprValidate):
    ...
    **Be careful when accessing fields**, because only the subclasses guarantee there existance
    ...
    blocks: list[SRBlock]     | None = no_field()
    block: SRBlock            | None = no_field()
    immediate: str | bool     | None = no_field()
    dropdown: SRDropdownValue | None = no_field()
...
class SRBlockAndTextInputValue(SRInputValue):
    ...
    blocks: None = no_field() 
    block: SRBlock | None
    immediate: str
    dropdown: None = no_field()

    post_validate = SRInputValue._validate_block

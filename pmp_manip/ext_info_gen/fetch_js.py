...
from pmp_manip.utility            import (
    ...
    MANIP_NetworkFetchError, MANIP_UnexpectedFetchError, MANIP_FileFetchError, GU_FileNotFoundError,
)
...
def fetch_js_code(source: str, tolerate_file_path: bool) -> str:
    ...
    Raises:
        ...
        GU_FileNotFoundError: If the local file does not exist
    ...
    if   isinstance(source, str) and source.startswith("data:"):
        ...
        if not path.exists(source):
            raise GU_FileNotFoundError(f"File not found: {source}")
        try:

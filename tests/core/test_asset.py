...
SIMPLE_SVG_EXAMPLE = b'<?xml version="1.0" encoding="UTF-8"?>\n<svg width="100" height="100" xmlns="http://www.w3.org/2000/svg">\n  <circle cx="50" cy="50" r="40" fill="red" />\n</svg>'
...
def test_FRCostume_step_invalid_format():
    ...
    with raises(ThanksError):
        srcostume = frcostume.step(asset_files={
...
def test_FRSound_step():
    ...
    assert audio_segment_equal(srsound.content, expected)
...
def test_SRBitmapCostume_eq_different_bitmap_resolution(config, bitmap_example):
    a = SRBitmapCostume(
        name="my costume",
        file_extension="png",
        rotation_center=(0, 0),
...
"""
def test_SRSound_eq_equal(sound_example):
    a = SRSound(
        name="my sound",
        file_extension="wav",
    ...
    b = SRSound(
        name="my sound",
        file_extension="wav",
        content=copy(sound_example),
    )
    assert a.content != b.content
    assert a == b

def test_SRSound_eq_different_name(sound_example):
    a = SRSound(
        name="my first sound",
        file_extension="wav",
        content=sound_example,
    ...
    b = SRSound(
        name="my second sound",
        file_extension="wav",
        content=sound_example,
    )
    assert a != b

def test_SRSound_eq_different_file_extension(sound_example):
    a = SRSound(
        name="my sound",
        file_extension="wav",
        content=sound_example,
...
def test_SRSound_eq_different_content(sound_example):
    a = SRSound(
        name="my sound",
        file_extension="wav",
        content=sound_example,
    ...
    b = SRSound(
        name="my sound",
        file_extension="mp3",
        content=sound_example.speedup(playback_speed=0.5),
    )
    assert a != b
"""

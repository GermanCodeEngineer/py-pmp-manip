...
def test_FRCostume_step_bitmap():
    ...
    srcostume = frcostume.step(asset_files={"05630bfa94501a3e5d61ce443a0cea70.png": SIMPLE_PNG_EXAMPLE})
...
def test_FRSound_step():
    ...
    srsound = frsound.step(asset_files={"83a9787d4cb6f3b7632b4ddfebf74367.wav": SIMPLE_WAV_EXAMPLE}) # TODO
...
def test_SRVectorCostume_create_empty(config):
    ...
    assert isinstance(srcostume.content, ElementTree.Element)
    assert ElementTree.tostring(srcostume.content, encoding="unicode") == EMPTY_SVG_COSTUME_XML
...
def test_SRBitmapCostume_validate(config):
    content = Image.open(BytesIO(SIMPLE_PNG_EXAMPLE))
    content.load()

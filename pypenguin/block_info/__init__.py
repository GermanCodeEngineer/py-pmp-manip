from typing import Iterable, TYPE_CHECKING

from block_info.basis import *

if TYPE_CHECKING:
    from block_mutation import SRMutation

class OpcodeInfoApi:
    _grepr = True
    _grepr_fields = ["block_info_sets"]

    block_info_sets: list[CategoryOpcodesInfo]
    
    def __init__(self):
        self.block_info_sets = []

    def add_block_info_set(self, block_info_set: CategoryOpcodesInfo):
        for other_set in self.block_info_sets:
            if other_set.name == block_info_set.name:
                raise ValueError(f"A CategoryOpcodesInfo called {repr(block_info_set)} was alredy added")
        self.block_info_sets.append(block_info_set)

    def get_set_by_name(self, name: str) -> CategoryOpcodesInfo:
        for block_info_set in self.block_info_sets:
            if block_info_set.name == name:
                return block_info_set
        raise ValueError(f"Couldn't find CategoryOpcodesInfo {repr(name)}")
    
    def get_sets_by_prefix(self, prefix: str) -> Iterable[CategoryOpcodesInfo]:
        for block_info_set in self.block_info_sets:
            if block_info_set.uses_prefix(prefix):
                yield block_info_set
        
    def get_info_by_opcode(self, opcode: str, default_none: bool = False) -> OpcodeInfo | None:
        opcode_prefix = opcode[:opcode.index("_")]
        main_opcode = opcode[opcode.index("_")+1:]
        generator = self.get_sets_by_prefix(opcode_prefix)
        for block_info_set in generator:
            block_info = block_info_set.get_info_by_opcode(main_opcode, default_none=True)
            if block_info is not None:
                return block_info
        if default_none:
            return None
        raise ValueError(f"Couldn't find OpcodeInfo by opcode {repr(opcode)}")
    
    def get_info_by_new_opcode(self, new_opcode: str, mutation: "SRMutation | None", default_none: bool = False) -> OpcodeInfo | None:
        for block_info_set in self.block_info_sets:
            block_info = block_info_set.get_info_by_new_opcode(new_opcode, mutation, default_none=True)
            print(block_info_set.name, block_info)
            if block_info is not None:
                return block_info
        if default_none:
            return None
        raise ValueError(f"Couldn't find OpcodeInfo by new opcode {repr(new_opcode)}")

    def info_exists_for_new_opcode(self, mutation: "SRMutation | None", opcode: str) -> bool:
        return self.get_info_by_new_opcode(opcode, default_none=True) is not None

from block_info.motion    import motion
from block_info.looks     import looks
from block_info.sounds    import sounds
from block_info.events    import events
from block_info.control   import control
from block_info.sensing   import sensing
from block_info.operators import operators
from block_info.variables import variables
from block_info.lists     import lists
from block_opcodes        import *

motion.add_block("goto_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#REACHABLE TARGET MENU (GO)",
))
motion.add_block("glideto_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#REACHABLE TARGET MENU (GLIDE)",
))
motion.add_block("pointtowards_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#OBSERVABLE TARGET MENU",
))

looks.add_block("costume", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#COSTUME MENU",
))
looks.add_block("backdrops", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#BACKDROP MENU",
))
looks.add_block("getinput_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#COSTUME PROPERTY MENU",
))
looks.add_block("changeVisibilityOfSprite_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#SHOW/HIDE SPRITE MENU",
))
looks.add_block("getOtherSpriteVisible_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#IS SPRITE VISIBLE MENU",
))

sounds.add_block("sounds_menu", OpcodeInfo( # this is certainly correct.
    block_type=BlockType.MENU,
    new_opcode="#SOUND MENU",
))

control.add_block("stop_sprite_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#STOP SPRITE MENU",
))
control.add_block("create_clone_of_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#CLONE TARGET MENU",
))
control.add_block("run_as_sprite_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#RUN AS SPRITE MENU",
))

sensing.add_block("touchingobjectmenu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#TOUCHING OBJECT MENU",
))
sensing.add_block("fulltouchingobjectmenu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#FULL TOUCHING OBJECT MENU",
))
sensing.add_block("touchingobjectmenusprites", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#TOUCHING OBJECT MENU SPRITES",
))
sensing.add_block("distancetomenu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#DISTANCE TO MENU",
))
sensing.add_block("keyoptions", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#KEY MENU",
))
sensing.add_block("scrolldirections", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#SCROLL DIRECTION MENU",
))
sensing.add_block("of_object_menu", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#OJBECT PROPERTY MENU",
))
sensing.add_block("fingeroptions", OpcodeInfo(
    block_type=BlockType.MENU,
    new_opcode="#FINGER INDEX MENU",
))
variables.add_block(OPCODE_VAR_VALUE.removeprefix("data_"), OpcodeInfo(
    block_type=BlockType.STRING_REPORTER,
    new_opcode=NEW_OPCODE_VAR_VALUE,
    dropdowns={
        "VARIABLE": DropdownInfo(DropdownType.VARIABLE, new="VARIABLE"),
    },
    can_have_monitor="True",
))
lists.add_block(OPCODE_LIST_VALUE.removeprefix("data_"), OpcodeInfo(
    block_type=BlockType.STRING_REPORTER,
    new_opcode=NEW_OPCODE_LIST_VALUE,
    dropdowns={
        "LIST": DropdownInfo(DropdownType.LIST, new="LIST"),
    },
    can_have_monitor="True",
))

def CB_OLD_OPCODE_HANDLER(new_opcode: str, mutation: "SRMutation | None") -> str | None:
    from block_mutation import SRCustomBlockMutation
    if new_opcode == NEW_OPCODE_CB_DEF:
        assert isinstance(mutation, SRCustomBlockMutation)
        if mutation.optype.is_reporter:
            return OPCODE_CB_DEF_RET
        else:
            return OPCODE_CB_DEF
    return None

custom_blocks = CategoryOpcodesInfo(
    name="Custom Blocks",
    opcode_prefix="procedures",
    alt_opcode_prefixes=["argument"],
    block_infos={
        OPCODE_CB_DEF.removeprefix("procedures_"): OpcodeInfo(
            block_type=BlockType.HAT,
            new_opcode=NEW_OPCODE_CB_DEF,
        ),
        OPCODE_CB_DEF_RET.removeprefix("procedures_"): OpcodeInfo(
            block_type=BlockType.HAT,
            new_opcode=NEW_OPCODE_CB_DEF,
        ),
        OPCODE_CB_PROTOTYPE.removeprefix("procedures_"): OpcodeInfo( # only temporary
            block_type=BlockType.NOT_RELEVANT,
            new_opcode="#CUSTOM BLOCK PROTOTYPE",
        ),
        OPCODE_CB_CALL.removeprefix("procedures_"): OpcodeInfo(
            block_type=BlockType.DYNAMIC,
            new_opcode="call custom block",
        ),
        "return": OpcodeInfo(
            block_type=BlockType.ENDING_STATEMENT,
            new_opcode="return (VALUE)",
            inputs={
                "return": InputInfo(InputType.TEXT, new="VALUE"),
            },
        ),
        "set": OpcodeInfo(
            block_type=BlockType.STATEMENT,
            new_opcode="set (PARAM) to (VALUE)",
            inputs={
                "PARAM": InputInfo(InputType.ROUND, new="PARAM"),
                "VALUE": InputInfo(InputType.TEXT, new="VALUE"),
            },
        ),
        OPCODE_CB_ARG_TEXT.removeprefix("argument_"): OpcodeInfo(
            block_type=BlockType.STRING_REPORTER,
            new_opcode="value of text [ARGUMENT]",
            alt_opcode_prefix="argument",
        ),
        OPCODE_CB_ARG_BOOL.removeprefix("argument_"): OpcodeInfo(
            block_type=BlockType.BOOLEAN_REPORTER,
            new_opcode="value of boolean [ARGUMENT]",
            alt_opcode_prefix="argument",
        ),
    },
    get_old_opcode_handler=CB_OLD_OPCODE_HANDLER,
)





info_api = OpcodeInfoApi()
info_api.add_block_info_set(motion       )
info_api.add_block_info_set(looks        )
info_api.add_block_info_set(sounds       )
info_api.add_block_info_set(events       )
info_api.add_block_info_set(control      )
info_api.add_block_info_set(sensing      )
info_api.add_block_info_set(operators    )
info_api.add_block_info_set(variables    )
info_api.add_block_info_set(lists        )
info_api.add_block_info_set(custom_blocks)

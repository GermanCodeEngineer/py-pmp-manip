...
from pypenguin.utility.errors import PP_TypeValidationError, PP_RangeValidationError, PP_InvalidValueError
...
def AA_TYPE(obj, path, attr, t, condition=None) -> None:
    ...
    if not isinstance(attr_value, t):
        raise PP_TypeValidationError(path, f"{descr} must be of type {_repr_type(t)} not {_repr_type(attr_value.__class__)}", condition)
...
def AA_TYPES(obj, path, attr, ts, condition=None) -> None:
    ...
    if not isinstance(attr_value, ts):
        ...
        raise PP_TypeValidationError(path, f"{descr} must be one of types {types_str} not {_repr_type(attr_value.__class__)}", condition)

def AA_NONE(obj, path, attr, condition=None) -> None:
    attr_value, descr = _value_and_descr(obj, attr)
    if attr_value is not None:
        raise PP_TypeValidationError(path, f"{descr} must be None", condition)

def AA_NONE_OR_TYPE(obj, path, attr, t, condition=None) -> None:
    attr_value, descr = _value_and_descr(obj, attr)
    if (attr_value is not None) and not(isinstance(attr_value, t)):
        raise PP_TypeValidationError(path, f"{descr} must be either None or of type {_repr_type(t)} not of type {_repr_type(attr_value.__class__)}", condition)

def AA_LIST_OF_TYPE(obj, path, attr, t, condition=None) -> None:
    attr_value, descr = _value_and_descr(obj, attr)
    msg = f"{descr} must be a list of {_repr_type(t)}"
    if not isinstance(attr_value, list):
        raise PP_TypeValidationError(path, f"{msg} not a {_repr_type(attr_value.__class__)}", condition)
    for item in attr_value:
        if not isinstance(item, t):
            raise PP_TypeValidationError(path, f"{msg} not of {_repr_type(item.__class__)}", condition)
...
def AA_DICT_OF_TYPE(obj, path, attr, key_t, value_t, condition=None) -> None:
    ...
    if not isinstance(attr_value, dict):
        raise PP_TypeValidationError(path, f"{descr} must be a dict NOT a {_repr_type(attr_value.__class__)}. Each key must be of type {_repr_type(key_t)}. Each value must be of type {_repr_type(value_t)}", condition)
    for key, value in attr_value.items():
        if not isinstance(key, key_t):
            raise PP_TypeValidationError(path, f"{descr} must be a dict. Each key must be of type {_repr_type(key_t)} NOT {_repr_type(key.__class__)}. Each value must be of type {_repr_type(value_t)}", condition)
        if not isinstance(value, value_t):
            raise PP_TypeValidationError(path, f"{descr} must be a dict. Each key must be of type {_repr_type(key_t)}. Each value must be of type {_repr_type(value_t)} NOT {_repr_type(value.__class__)}", condition)

def AA_MIN(obj, path, attr, min, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if attr_value < min:
        raise PP_RangeValidationError(path, f"{descr} must be at least {min}", condition)

def AA_MAX(obj, path, attr, max, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if attr_value > max:
        raise PP_RangeValidationError(path, f"{descr} must be at most {max}", condition)

def AA_RANGE(obj, path, attr, min, max, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if (attr_value < min) or (attr_value > max):
        raise PP_RangeValidationError(path, f"{descr} must be at least {min} and at most {max}", condition)

def AA_MIN_LEN(obj, path, attr, min_len: int, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if len(attr_value) < min_len:
        raise PP_RangeValidationError(path, f"{descr} must contain at least {min_len} element(s)")

def AA_EXACT_LEN(obj, path, attr, length: int, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if len(attr_value) != length:
        raise PP_RangeValidationError(path, f"{descr} must contain exactly {length} element(s)")
...
def AA_COORD_PAIR(obj, path, attr, condition=None):
    ...
    if (
        ...
        raise PP_TypeValidationError(path, f"{descr} must be a coordinate pair. It must be a tuple of length 2. Each item must be an int or float", condition)
...
def AA_BOXED_COORD_PAIR(
    ...
    if (
        ...
        raise PP_TypeValidationError(path, msg, condition)
    if (
        ...
        raise PP_RangeValidationError(path, msg, condition)
...
def AA_JSON_COMPATIBLE(obj, path, attr, condition=None):
    ...
    try:
        ...
        error = PP_TypeValidationError(path, f"{descr} must be JSON-compatible", condition)
...
def AA_EQUAL(obj, path, attr, value, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if attr_value != value:
        raise PP_InvalidValueError(path, f"{descr} must be {repr(value)}", condition)

def AA_NOT_EQUAL(obj, path, attr, value, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if attr_value == value:
        raise PP_InvalidValueError(path, f"{descr} must NOT be {repr(value)}", condition)
...
def AA_BIGGER_OR_EQUAL(obj, path, attr1, attr2, condition=None):
    ...
    if not(attr1_value >= attr2_value):
        raise PP_RangeValidationError(path, f"{attr1_descr} must be bigger then or equal to {attr2}", condition)

def AA_NOT_ONE_OF(obj, path, attr, forbidden_values, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    if attr_value in forbidden_values:
        raise PP_InvalidValueError(path, f"{descr} must not be one of {repr(forbidden_values)}")

def AA_HEX_COLOR(obj, path, attr, condition=None):
    attr_value, descr = _value_and_descr(obj, attr)
    msg = f"{descr} must be a valid hex color eg. '#FF0956'"
    if not isinstance(attr_value, str):
        raise PP_TypeValidationError(path, msg)
    if not bool(re.fullmatch(r'#([0-9a-fA-F]{6})', attr_value)):
        raise PP_InvalidValueError(path, msg)
